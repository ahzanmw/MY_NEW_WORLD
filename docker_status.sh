...
#ABC= docker inspect -f {{.State.Running}} ardsliteroutingengine|tr -d '\n' ;
##ABC= "$(echo "$ABC"|tr -d '\n')";
##ABC= tr -d "$ABC";
#echo $ABC;
#if [[ `docker inspect -f {{.State.Running}} ardsliteroutingengine` -eq "false" ]]; then
#echo "asdasdasdadasda";
#fi
URL="http://162.243.81.39";
...
curl "$URL/PHPMailer/PHPMailer/examples/sendmail.php?containers=$DOCKER&server=$IP";
fi
